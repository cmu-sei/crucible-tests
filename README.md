...
- **Every seeded spec needs a seeding client that this realm has.** `player-helpers.ts` gets its token from `player.vm.api`, which is in the Aspire dev realm and *not* in the helm realm — that one ships `player.ui`, `player.vm.ui`, `player.vm.console.ui`, `player.vm.swagger`, `player.vm.admin` and `player.vm.webhooks`. Left alone, every `beforeAll` that seeds a view fails with a 401 on the token request (which names this cause in the error). Set `PLAYER_SEEDING_CLIENT_ID` to a client that allows a direct access grant and carries both the `player` and `player-vm` audiences, and `PLAYER_SEEDING_SCOPE` if its scopes are named differently. This affects `player/`, `playerVm/` and `steamfitter/`.
...
- `playerVm/tests/contract/` reads the sibling application repositories (`player/vm.api`, `player/vm.ui`, `player/console.ui`) rather than a running service, and skips when they are not checked out. Point `CRUCIBLE_SOURCE_ROOT` at the directory holding them if they are somewhere other than this repo's parent; setting it (or `CRUCIBLE_REQUIRE_CONTRACTS=1`) also turns those skips into failures, which is what a pipeline that does check them out should do. Note that `CI=1` deliberately does *not* — a job cloning only this repo cannot satisfy them. The one exception is `openapi-surface.spec.ts`, which compares the checked-in client against the document the VM API is *serving* (`/swagger/v1/swagger.json`, the same one `npm run swagger:gen` is pointed at) because `vm.api` no longer publishes a snapshot of it. Those tests need the VM API up and are a `requirePrecondition` skip when nothing answers, so `CI=1` does escalate them.
