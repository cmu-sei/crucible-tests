...
test.describe('Export and Import', () => {
  test('Export MSEL to CSV', async ({ blueprintAuthenticatedPage: page }) => {
    ...
    await expect(page.getByRole('link', { name: 'HSEEP' })).toBeVisible({ timeout: 5000 });

    // 2. Click 'Download' button for HSEEP MSEL
    const downloadButton = page.getByRole('button', { name: 'Download HSEEP' });
    ...
    // expect: XLSX file is generated and downloaded
    const download = await downloadPromise;
    const downloadPath = await download.path();

    expect(downloadPath).toBeTruthy();
    expect(download.suggestedFilename()).toMatch(/\.xlsx$/i);
    expect(download.suggestedFilename()).toContain('HSEEP');

    // expect: XLSX file contains data
    if (downloadPath) {
