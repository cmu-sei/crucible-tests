...
// ========================================================================
// Theme helpers for dark-mode testing
// ========================================================================

/**
 * Themes every Gallery functional spec is parameterized over. Each spec runs once
...
/** True when the Gallery UI is currently rendering in dark mode. */
export async function galleryIsDarkTheme(page: Page): Promise<boolean> {
  return page.evaluate(() => document.body.classList.contains('darkMode'));
}

/**
 * Put the Gallery UI into the requested theme via the top-bar "Admin User" menu's
 * "Dark Theme" switch, then wait until <body> reflects the change.
 *
 * Gallery persists the selected theme through @cmusei/crucible-common's auth store
 * (localStorage key 'akita-gallery-ui'), so a theme set here survives the
 ...
 */
export async function applyGalleryTheme(page: Page, theme: GalleryTheme): Promise<void> {
  const wantDark = theme === 'dark';
  if ((await galleryIsDarkTheme(page)) === wantDark) {
    return;
  }
  ...
  await page.locator('body').evaluate(
    (body, expected) =>
      new Promise<void>((resolve, reject) => {
        ...
        setTimeout(() => {
          observer.disconnect();
          reject(new Error(`Gallery theme did not become ${expected ? 'dark' : 'light'}`));
        }, 10000);
      }),
    wantDark
  );
