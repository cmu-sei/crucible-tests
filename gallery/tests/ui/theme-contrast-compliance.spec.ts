...
import { test, expect, Page, Locator, Services, GALLERY_THEMES, applyGalleryTheme } from '../../fixtures';

/**
 * UI — real WCAG contrast on the Gallery admin surface, in both themes.
 *
 * The sibling `dark-theme.spec.ts` only asserts that the theme toggle works; it
 * never measures a contrast ratio, so a theme that rendered dark-grey-on-black
 * would still pass it. This spec closes that gap: it samples the colours the app
 ...
 * "Add Collection" button), so it needs no cleanup.
 */
...
for (const theme of GALLERY_THEMES) {
  test.describe(`${theme} theme › UI`, () => {
    // WCAG 1.4.3 — text contrast
    test('Theme Contrast Compliance (WCAG 1.4.3 text)', async ({ galleryAuthenticatedPage: page }) => {
      ...
      // Navigate to the admin Collections page (has heading + table always present)
      await page.goto(`${Services.Gallery.UI}/?section=collections`);
    ...
    // "Add Collection" button draws its icon in the brand accent colour; both themes
    ...
    // specific hex.
    test('Primary action icon contrast (WCAG 1.4.11)', async ({ galleryAuthenticatedPage: page }) => {
      await applyGalleryTheme(page, theme);
      await page.goto(`${Services.Gallery.UI}/?section=collections`);
      await expect(page.getByRole('table')).toBeVisible();

      // The Collections admin uses `title` attribute for its add button
      const addButton = page.getByRole('button', { name: 'Add Collection' });
      await expect(addButton).toBeVisible();

      // Non-text contrast bar is a flat 3:1 regardless of size.
      const s = await sample(addButton);
      expect(s.surface, 'Add Collection icon: no painted background found').not.toBeNull();
      const ratio = contrastRatio(s.color, s.surface!);
      expect(
        ratio,
        `${theme}: "Add Collection" icon contrast ${ratio.toFixed(2)}:1 ` +
