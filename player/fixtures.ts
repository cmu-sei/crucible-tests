...
/**
 * Themes every Player functional spec is parameterized over. Each spec runs once
...
/** True when the Player UI is currently rendering in dark mode. */
...
/**
 * Put the Player UI into the requested theme via the top-bar "Menu" button's
 * "Dark Theme" switch, then wait until <body> reflects the change.
 *
 * Player persists the selected theme through @cmusei/crucible-common's auth store
 * (localStorage), so a theme set here survives the client-side navigations a test
 * performs afterward — it only needs applying once, right after authentication.
 * A fresh browser context defaults to light, so requesting 'light' is a no-op.
 *
 ...
 */
export async function applyPlayerTheme(page: Page, theme: PlayerTheme): Promise<void> {
  const wantDark = theme === 'dark';
  if ((await playerIsDarkTheme(page)) === wantDark) {
    return;
  }
  ...
  await page.locator('body').evaluate(
    (body, expected) =>
      new Promise<void>((resolve, reject) => {
        ...
        setTimeout(() => {
          observer.disconnect();
          reject(new Error(`Player theme did not become ${expected ? 'dark' : 'light'}`));
        }, 10000);
      }),
    wantDark
  );
