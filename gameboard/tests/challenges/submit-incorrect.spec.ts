...
// Incorrect submission = challenge with score < points. We seed a challenge
// with a partial score to simulate an attempted-but-incorrect state, then
// verify the detail reports score < points.
test.describe('Challenges', () => {
  ...
  test.beforeEach(async () => {
    ...
    sponsor = await createSponsor(gbToken, `IncSp-${Date.now()}`);
    ({ previousSponsorId } = await setAdminSponsor(gbToken, sponsor.id));
    workspace = await createWorkspace(tmToken, { name: `Inc-WS-${Date.now()}`, audience: 'gameboard' });
    game = await createGame(gbToken, { name: `Inc-${Date.now()}`, startOffsetDays: -1, endOffsetDays: 30 });
    const spec = await addChallengeSpecToGame(gbToken, game.id, { externalId: workspace.id, name: 'Incorrect' });
    specId = spec.id;
    ...
    try {
      const ch = await seedChallenge(client, {
        ...
        externalId: workspace.id, name: 'Incorrect', points: 500, score: 0,
      });
  ...
  test('Submit Challenge Answer - Incorrect', async ({ gameboardAuthenticatedPage: page }) => {
