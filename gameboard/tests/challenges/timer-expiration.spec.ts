...
// A challenge where EndTime < now simulates timer-expiration. We seed one
// directly via the DB (duration = -1 minute) and verify the detail reports
// an expired window.
test.describe('Challenges', () => {
  ...
  test.beforeEach(async () => {
    ...
    sponsor = await createSponsor(gbToken, `TimSp-${Date.now()}`);
    ({ previousSponsorId } = await setAdminSponsor(gbToken, sponsor.id));
    workspace = await createWorkspace(tmToken, { name: `Tim-WS-${Date.now()}`, audience: 'gameboard' });
    game = await createGame(gbToken, { name: `Tim-${Date.now()}`, startOffsetDays: -1, endOffsetDays: 30 });
    const spec = await addChallengeSpecToGame(gbToken, game.id, { externalId: workspace.id, name: 'Expired' });
    specId = spec.id;
    ...
    try {
      const ch = await seedChallenge(client, {
        ...
        externalId: workspace.id, name: 'Expired', points: 100,
        durationMinutes: -1, // ends in the past
      });
  ...
  test('Challenge Timer - Time Expiration', async ({ gameboardAuthenticatedPage: page }) => {
    ...
    try {
      ...
      expect(new Date(detail.endTime).getTime()).toBeLessThan(Date.now());
    } finally {
      await ctx.dispose();
    }
