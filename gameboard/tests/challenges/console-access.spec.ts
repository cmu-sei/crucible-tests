...
// Seed a challenge with a deployed gamespace and VM. Verify Gameboard's
// /api/challenge/consoles endpoint acknowledges our challenge (it returns a
// list of active consoles an admin can observe; for a seeded in-progress
// challenge this should include our challenge id).
test.describe('Challenges', () => {
  ...
  test.beforeEach(async () => {
    ...
    sponsor = await createSponsor(gbToken, `Con-${Date.now()}`);
    ({ previousSponsorId } = await setAdminSponsor(gbToken, sponsor.id));
    workspace = await createWorkspace(tmToken, { name: `Con-WS-${Date.now()}`, audience: 'gameboard' });
    game = await createGame(gbToken, { name: `Con-${Date.now()}`, startOffsetDays: -1, endOffsetDays: 30 });
    const spec = await addChallengeSpecToGame(gbToken, game.id, { externalId: workspace.id, name: 'Console' });
    specId = spec.id;
    ...
    try {
      const stateJson = JSON.stringify({
        id: 'seed', name: 'Console',
        challenge: { text: 'Console challenge', maxAttempts: 3, maxPoints: 100, attempts: 0, score: 0, sectionCount: 1, sectionIndex: 0, questions: [] },
        vms: [{ id: 'vm-console', name: 'kali', isolationId: 'iso-c' }],
        isActive: true,
      });
      const ch = await seedChallenge(client, {
        gameId: game.id, specId, playerId, teamId,
        externalId: workspace.id, name: 'Console', points: 100,
