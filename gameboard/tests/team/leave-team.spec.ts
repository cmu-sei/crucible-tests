...
test.describe('Team Management', () => {
  ...
  test.beforeEach(async () => {
    ...
    sponsor = await createSponsor(gbAdmin, `Leave-${Date.now()}`);
  ...
  test('Leave Team - Team Member', async ({ gameboardAuthenticatedPage: page }) => {
    const before = await listGamePlayers(gbAdmin, game.id, captain.player.teamId);
    expect(before.length).toBe(2);

    // Member unenrolls themselves.
    await deletePlayer(member!.token, member!.player.id);
    member = null;
