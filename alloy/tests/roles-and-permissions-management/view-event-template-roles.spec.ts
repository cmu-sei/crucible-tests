...
test.describe('Roles and Permissions Management', () => {
  test('View Event Template Roles', async ({ page }) => {
    ...
    // 2. Click on 'EventTemplate Roles' tab
    await page.getByRole('tab', { name: 'EventTemplate Roles', exact: true }).click();

    // expect: Event template roles list is displayed
    await expect(page.getByRole('tab', { name: 'EventTemplate Roles', exact: true })).toHaveAttribute('aria-selected', 'true');
  });
});
