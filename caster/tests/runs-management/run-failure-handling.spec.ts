...
test.describe('Runs Management (Plan, Apply, Destroy)', () => {
  test('Run Failure Handling', async ({ casterAuthenticatedPage: page, cleanupCasterProject }) => {
    const projectName = `Fail-Run-Project-${Date.now()}`;
    ...
    await page.getByRole('textbox', { name: 'Name' }).fill('Fail-Run-Dir');
    await page.getByRole('button', { name: 'Save' }).click();
    await expect(page.getByRole('button', { name: 'Fail-Run-Dir' })).toBeVisible({ timeout: 10000 });

    await page.getByRole('button', { name: 'Fail-Run-Dir' }).click();
