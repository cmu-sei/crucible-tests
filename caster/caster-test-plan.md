...
**Steps:**
  ...
  3. POST /api/files with a name containing either path separator, a UNC path, a null byte, a control character, leading or trailing whitespace, or more than 255 characters, and with the names `.` and `..`
  ...
  5. POST /api/files with names that cannot escape the working directory, such as `main.tf`, `my file.tf`, `network (old).tf`, `café.tf`, a percent encoded `%2e%2e%2fescaped.tf`, and a 255 character name
