...
- **`shared-fixtures.ts` → `waitForPageFunction(page, fn, arg, opts)`** — **use this instead of `page.waitForFunction()`, always.** `page.waitForFunction` is silently broken in Firefox on every Crucible UI: Angular's zone.js replaces the page's global `Promise`, and Firefox's Juggler protocol doesn't await the resulting `ZoneAwarePromise`, so the call returns *immediately* without ever testing the predicate (`waitForFunction(() => false, …, {timeout: 4000})` resolves in 26 ms) and reading its value yields `{__zone_symbol__state, __zone_symbol__value}` instead of the value. Chromium is unaffected, so the damage shows up as a Firefox-only failure or — worse — a Firefox-only vacuous pass. `page.evaluate()` is fine; the helper polls it. Full evidence is in the helper's doc comment.
