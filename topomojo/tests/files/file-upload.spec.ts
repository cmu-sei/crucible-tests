...
test.describe('File Management', () => {
  test('File Upload', async ({ topomojoAuthenticatedPage: page }) => {
    ...
    if (itemCount > 0) {
      ...
      const uploadButton = page.locator('button:has-text("Upload"), [class*="upload"], input[type="file"]').first();
      const hasUpload = await uploadButton.isVisible({ timeout: 5000 }).catch(() => false);

      if (hasUpload) {
        // expect: File selection interface is available
