...
test.describe('Template Management', () => {
  test('Template List Display', async ({ topomojoAuthenticatedPage: page }) => {
    ...
    if (itemCount > 0) {
      ...
      const templateList = page.locator('[class*="template"], mat-expansion-panel, mat-list, [class*="vm-list"]').first();
      const hasList = await templateList.isVisible({ timeout: 10000 }).catch(() => false);
      if (hasList) {
